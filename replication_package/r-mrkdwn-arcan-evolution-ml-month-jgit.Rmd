...
pkg.merged.df <- read.csv("cache/jgit/package_merged.csv",
                          head=TRUE,
                          sep=",")
...
# Add missing column UD copying the number of correlated packages. It will by transformed to a factor below
    ...
    HLmeanFanIn, HLsdFanIn, HLmeanFanOut, HLsdFanOut, HLmeanTD, HLsdTD, InstabilityUnstableDependenciesPackage,
    RMA, RMI, CE, CA, RMD))
```
...
set.seed(825)
confusionMatricesNB <- runCV(pkg.dataset.nzv, models, targets, "jgit-month-")
write.csv(confusionMatricesNB, "jgit-month-cm-ALL-nb.csv")
#```

#```{r train C5.0 and all targets}
models <- c("C5.0") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesC50 <- runCV(pkg.dataset.nzv, models, targets, "jgit-month-")
write.csv(confusionMatricesC50, "jgit-month-cm-ALL-c50.csv")
#```
...
set.seed(825)
confusionMatricesRF <- runCV(pkg.dataset.nzv, models, targets, "jgit-month-")
write.csv(confusionMatricesRF, "jgit-month-cm-ALL-rf.csv")
#```

#```{r write all confusion matrices}
write.csv(rbind(confusionMatricesNB, confusionMatricesC50, confusionMatricesRF), "jgit-month-cm-ALL-ALL.csv")
```

```{r train nb and all targets smote}
models <- c("nb") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesNB <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.jgit-month-")
write.csv(confusionMatricesNB, "smote.jgit-month-cm-ALL-nb.csv")
#```

#```{r train C5.0 and all targets}
models <- c("C5.0") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesC50 <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.jgit-month-")
write.csv(confusionMatricesC50, "smote.jgit-month-cm-ALL-c50.csv")
#```
...
models <- c("rf") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesRF <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.jgit-month-")
write.csv(confusionMatricesRF, "smote.jgit-month-cm-ALL-rf.csv")
#```

#```{r write all confusion matrices}
write.csv(rbind(confusionMatricesNB, confusionMatricesC50, confusionMatricesRF), "smote.jgit-month-cm-ALL-ALL.csv")
```
...
confusionMatricesSVMRadial <- runCV.svm(pkg.dataset.nzv, grid, models, targets, "jgit-month-")
write.csv(confusionMatricesSVMRadial, "jgit-month-cm-ALL-svm-radial.csv")
```
...
confusionMatricesSVMlinear <- runCV.svm(pkg.dataset.nzv, grid, models, targets, "jgit-month-")
write.csv(confusionMatricesSVMlinear, "jgit-month-cm-ALL-svm-linear.csv")
```
...
models <- c("JRip")
set.seed(825)
trainjrip <- run50CV(pkg.dataset.nzv.binary, models, targets, "jgit-")
# write.csv(pkg.dataset.nzv.binary, "tomcat-cm-lagged-week.csv")
    ...
    write(as.character(summary(c50.1)), "data/rule/jgit-jrip-month-c50.star.txt")
    write(as.matrix(scan(text = .jcall(c50.1$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/jgit-jrip-month-c50.star.txt", append = TRUE)
    write(as.character(summary(c50.2)), "data/rule/jgit-jrip-month-c50.circle.txt")
    write(as.matrix(scan(text = .jcall(c50.2$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/jgit-jrip-month-c50.circle.txt", append = TRUE)
    write(as.character(summary(c50.3)), "data/rule/jgit-jrip-month-c50.clique.txt")
    write(as.matrix(scan(text = .jcall(c50.3$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/jgit-jrip-month-c50.clique.txt", append = TRUE)
    write(as.character(summary(c50.4)), "data/rule/jgit-jrip-month-c50.chain.txt")
    write(as.matrix(scan(text = .jcall(c50.4$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/jgit-jrip-month-c50.chain.txt", append = TRUE)
    write(as.character(summary(c50.5)), "data/rule/jgit-jrip-month-c50.cd.txt")
    write(as.matrix(scan(text = .jcall(c50.5$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/jgit-jrip-month-c50.cd.txt", append = TRUE)
    write(as.character(summary(c50.6)), "data/rule/jgit-jrip-month-c50.ud.txt")
    write(as.matrix(scan(text = .jcall(c50.6$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/jgit-jrip-month-c50.ud.txt", append = TRUE)
    write(as.character(summary(c50.7)), "data/rule/jgit-jrip-month-c50.hl.txt")
    write(as.matrix(scan(text = .jcall(c50.7$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/jgit-jrip-month-c50.hl.txt", append = TRUE)
    write(as.character(summary(c50.8)), "data/rule/jgit-jrip-month-c50.ixpd.txt")
    write(as.matrix(scan(text = .jcall(c50.8$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/jgit-jrip-month-c50.ixpd.txt", append = TRUE)
```
...
# write.csv(pkg.dataset.nzv.binary, "tomcat-cm-lagged-week.csv")
    ...
    c50.7 <- trainjrip$`8`$finalModel #hl
    c50.8 <- trainjrip$`9`$finalModel #ixpd
    write(as.character(summary(c50.1)), "data/rule.smote/jgit-jrip-month-c50.star.txt")
    write(as.matrix(scan(text = .jcall(c50.1$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/jgit-jrip-month-c50.star.txt", append = TRUE)
    write(as.character(summary(c50.2)), "data/rule.smote/jgit-jrip-month-c50.circle.txt")
    write(as.matrix(scan(text = .jcall(c50.2$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/jgit-jrip-month-c50.circle.txt", append = TRUE)
    write(as.character(summary(c50.3)), "data/rule.smote/jgit-jrip-month-c50.clique.txt")
    write(as.matrix(scan(text = .jcall(c50.3$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/jgit-jrip-month-c50.clique.txt", append = TRUE)
    write(as.character(summary(c50.4)), "data/rule.smote/jgit-jrip-month-c50.chain.txt")
    write(as.matrix(scan(text = .jcall(c50.4$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/jgit-jrip-month-c50.chain.txt", append = TRUE)
    write(as.character(summary(c50.5)), "data/rule.smote/jgit-jrip-month-c50.cd.txt")
    write(as.matrix(scan(text = .jcall(c50.5$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/jgit-jrip-month-c50.cd.txt", append = TRUE)
    write(as.character(summary(c50.6)), "data/rule.smote/jgit-jrip-month-c50.ud.txt")
    write(as.matrix(scan(text = .jcall(c50.6$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/jgit-jrip-month-c50.ud.txt", append = TRUE)
    write(as.character(summary(c50.7)), "data/rule.smote/jgit-jrip-month-c50.hl.txt")
    write(as.matrix(scan(text = .jcall(c50.7$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/jgit-jrip-month-c50.hl.txt", append = TRUE)
    write(as.character(summary(c50.8)), "data/rule.smote/jgit-jrip-month-c50.ixpd.txt")
    write(as.matrix(scan(text = .jcall(c50.8$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/jgit-jrip-month-c50.ixpd.txt", append = TRUE)
```
...
pkg.merged.df.sum <- ddply(pkg.merged.df[setdiff(names(pkg.merged.df), c("version", "commit_time"))],
    ...
    write.csv(file = "jgit-as-evolution.csv", pkg.merged.df.sum)
```