---
title: "AS evolution with arcan commons-math"
author: "RR"
...
# classmetrics <- read.csv(file="data/commons-math/CM.csv",colClasses=c('character','myDate','character','numeric','numeric','numeric','numeric'),head=TRUE,sep=",")
...
#to.timestamp.fromint <- function(d) as.POSIXct(d, origin = "1970-01-01", tz = "GMT")
# library(data.table)
classmetrics<-fread("data/commons-math/CM.csv")
...
# pkgmetrics <- read.csv(file="data/commons-math/PM.csv",colClasses=c('character','myDate','character','numeric','numeric','numeric','numeric','numeric'),head=TRUE,sep=",")
pkgmetrics <- fread("data/commons-math/PM.csv")
...
# hublike <- read.csv(file="data/commons-math/HL.csv",colClasses=c('character','myDate','character','numeric','numeric','numeric'),head=TRUE,sep=",")
hublike <- fread("data/commons-math/HL.csv")
...
# unstabledep <- read.csv(file="data/commons-math/UD.csv", colClasses=c('character', 'myDate', 'character', 'numeric', 'character', 'numeric'), head=TRUE, sep=",")
library(data.table)
unstabledep <- fread(file="data/commons-math/UD.csv")
...
clcycle <- fread("data/commons-math/classCyclesShapeTable.csv")
...
pkgcycle <- fread("data/commons-math/packageCyclesShapeTable.csv")
...
pkgcycle <- rename(pkgcycle, replace = c("ElementList" = "package"))
pkgcycle$CD <- 1
names(pkgcycle)
summary(pkgcycle)
```
...
```{r Reading IXPD csv}
#ixpd <- fread(file = "data/commons-math/IXPD.csv")
ixpd <- fread(file = "data/commons-math/cm5-ixpd.csv")
#library(stringr)
ixpd <- rename(ixpd, replace = c("java-out" = "java.out"))
ixpd <- rename(ixpd, replace = c("java-in" = "java.in"))
ixpd <- rename(ixpd, replace = c("commit-out" = "commit.out"))
ixpd <- rename(ixpd, replace = c("commit-in" = "commit.in"))
ixpd <- rename(ixpd, replace = c("ratio-out" = "ratio.out"))
ixpd <- rename(ixpd, replace = c("ratio-in" = "ration.in"))
ixpd <- ixpd[, java.out := gsub("^.*\\.(java|test)\\.", "", java.out),]
ixpd <- ixpd[, java.in := gsub("^.*\\.(java|test)\\.", "", java.in),]
...
```{r developers info}
class.dev = fread("data/commons-math/development.csv")
class.dev$commit_time <- to.timestamp.fromint(class.dev$commit_time)
class.dev <- class.dev[, javafile := gsub("^.*\\.(java|test|mantissa.src|mantissa.tests-src|experimental)\\.", "", javafile),]
...
##merging by metrics and cycle at package level (left outer join) qui
...
fwrite(pkg.metric.CD.HL.UD.ixpd, "cache/commons-math/package_merged.csv", append = FALSE)
fwrite(class.cycle.metric.ixpd.HL.UD.merging, "cache/commons-math/class_merged.csv", append = FALSE)
```
...
```{r write merged datasets to file}
fwrite(pkg.metric.CD.HL.UD.ixpd.dev, "cache/commons-math/package_merged.dev.csv", append = FALSE)
```
