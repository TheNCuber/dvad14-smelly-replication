...
# library(data.table)
# pkg.merged<-fread("cache/junit/package_merged.csv")
# pkg.merged[,.(commit_time=to.timestamp.fromint(pkg.merged$commit_time))]
# head(pkg.merged)

pkg.merged.df <- read.csv("cache/junit/package_merged.csv",
                          head=TRUE,
                          sep=",")
...
```{r group data by year-month-day, message=FALSE}
# library(plyr)
...
```{r lag package dataset, message=FALSE}
...
```{r train svm and all targets old}
shapes <- c("star", "circle", "tiny", "clique", "chain")
targets <- c(shapes, "CD", "UD", "HL", "IXPD")
...
range01 <- function(x) {(x - min(x)) / (max(x) - min(x)) }
    pkg.dataset.svm <- apply(pkg.dataset.svm, 2, range01)
    pkg.dataset.svm <- as.data.frame(pkg.dataset.svm)
	#names(pkg.dataset.svm) <- n
    head(pkg.dataset.svm)
# Remove columns with near-zero variance
pkg.dataset.nzv <- pkg.dataset.svm[, -nearZeroVar(pkg.dataset.svm)]
...
# define a function to encapsulate the training procedure
runCV.svm <- function(lagged, .models, .shapes, .outfilePrefix = "") {
        # Generate all combinations of targets and models to test
    #experiments <- expand.grid(shape = .shapes, model = .models)
    ...
    adply(experiments, .margins = 1, function(experiment) {
        ...
        if (currentShape %in% names(lagged)) {
			...
			#as.formula(paste(currentShape, " ~ .",sep = "")),
                ...
                method = currentModel,
                tuneGrid = grid, # 9 values of the cost function
...
confusionMatricesSVMRadial <- runCV.svm(pkg.dataset.nzv, grid, models, targets, "junit-month-")
write.csv(confusionMatricesSVMRadial, "junit-month-cm-ALL-svm-radial.csv")
```
...
confusionMatricesSVMlinear <- runCV.svm(pkg.dataset.nzv, grid, models, targets, "junit-month-")
write.csv(confusionMatricesSVMlinear, "junit-month-cm-ALL-svm-linear.csv")
...
set.seed(825)
confusionMatricesNB <- runCV(pkg.dataset.nzv, models, targets, "junit-month-")
write.csv(confusionMatricesNB, "junit-month-cm-ALL-nb.csv")
#```

#```{r train C5.0 and all targets}
models <- c("C5.0") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesC50 <- runCV(pkg.dataset.nzv, models, targets, "junit-month-")
write.csv(confusionMatricesC50, "junit-month-cm-ALL-c50.csv")
#```
...
set.seed(825)
confusionMatricesRF <- runCV(pkg.dataset.nzv, models, targets, "junit-month-")
write.csv(confusionMatricesRF, "junit-month-cm-ALL-rf.csv")
#```

#```{r write all confusion matrices}
write.csv(rbind(confusionMatricesNB, confusionMatricesC50, confusionMatricesRF), "junit-month-cm-ALL-ALL.csv")
```
  ...
  adply(experiments, .margins = 1, function(experiment) {
    ...
    if (currentShape %in% names(lagged)) {
      ...
      lagged.train <- cbind(lagged[c(currentShape)],
                            lagged[grep("-", names(lagged))])
        print(paste("Model:", currentModel))
        print(paste("Shape:", currentShape))
        print(names(lagged.train))
...
models <- c("nb") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesNB <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.junit-month-")
write.csv(confusionMatricesNB, "smote.junit-month-cm-ALL-nb.csv")
#```

#```{r train C5.0 and all targets}
models <- c("C5.0") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesC50 <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.junit-month-")
write.csv(confusionMatricesC50, "smote.junit-month-cm-ALL-c50.csv")
#```
...
models <- c("rf") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesRF <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.junit-month-")
write.csv(confusionMatricesRF, "smote.junit-month-cm-ALL-rf.csv")
#```

#```{r write all confusion matrices}
write.csv(rbind(confusionMatricesNB, confusionMatricesC50, confusionMatricesRF), "smote.junit-month-cm-ALL-ALL.csv")
```

[1] "Model: nb"
 [1] "star"          "numClasses-1"  "IXPD-1"        "numPackages-1" "tiny-1"        "chain-1"       "CD-1"          "numClasses-2"  "IXPD-2"        "tiny-2"       
[11] "chain-2"       "CD-2"          "numClasses-3"  "IXPD-3"        "tiny-3"        "chain-3"       "CD-3"          "numClasses-4"  "IXPD-4"        "numPackages-4"
[21] "tiny-4"        "chain-4"       "CD-4"          "numClasses-5"  "IXPD-5"        "numPackages-5" "tiny-5"        "chain-5"       "CD-5"          "numClasses-6" 
[31] "IXPD-6"        "numPackages-6" "tiny-6"        "chain-6"       "CD-6"          "numClasses-7"  "IXPD-7"        "tiny-7"        "chain-7"       "CD-7"         
[41] "numClasses-8"  "IXPD-8"        "tiny-8"        "chain-8"       "CD-8"          "numClasses-9"  "IXPD-9"        "tiny-9"        "chain-9"       "CD-9"         
[51] "numClasses-10" "IXPD-10"       "tiny-10"       "chain-10"      "CD-10"         "numClasses-11" "IXPD-11"       "tiny-11"       "chain-11"      "CD-11"        
[61] "numClasses-12" "IXPD-12"       "tiny-12"       "chain-12"      "CD-12"        
...
[1] "Model: nb"
 [1] "tiny"          "numClasses-1"  "IXPD-1"        "numPackages-1" "tiny-1"        "chain-1"       "CD-1"          "numClasses-2"  "IXPD-2"        "tiny-2"       
[11] "chain-2"       "CD-2"          "numClasses-3"  "IXPD-3"        "tiny-3"        "chain-3"       "CD-3"          "numClasses-4"  "IXPD-4"        "numPackages-4"
[21] "tiny-4"        "chain-4"       "CD-4"          "numClasses-5"  "IXPD-5"        "numPackages-5" "tiny-5"        "chain-5"       "CD-5"          "numClasses-6" 
[31] "IXPD-6"        "numPackages-6" "tiny-6"        "chain-6"       "CD-6"          "numClasses-7"  "IXPD-7"        "tiny-7"        "chain-7"       "CD-7"         
[41] "numClasses-8"  "IXPD-8"        "tiny-8"        "chain-8"       "CD-8"          "numClasses-9"  "IXPD-9"        "tiny-9"        "chain-9"       "CD-9"         
...
[1] "Model: nb"
 [1] "chain"         "numClasses-1"  "IXPD-1"        "numPackages-1" "tiny-1"        "chain-1"       "CD-1"          "numClasses-2"  "IXPD-2"        "tiny-2"       
[11] "chain-2"       "CD-2"          "numClasses-3"  "IXPD-3"        "tiny-3"        "chain-3"       "CD-3"          "numClasses-4"  "IXPD-4"        "numPackages-4"
[21] "tiny-4"        "chain-4"       "CD-4"          "numClasses-5"  "IXPD-5"        "numPackages-5" "tiny-5"        "chain-5"       "CD-5"          "numClasses-6" 
[31] "IXPD-6"        "numPackages-6" "tiny-6"        "chain-6"       "CD-6"          "numClasses-7"  "IXPD-7"        "tiny-7"        "chain-7"       "CD-7"         
[41] "numClasses-8"  "IXPD-8"        "tiny-8"        "chain-8"       "CD-8"          "numClasses-9"  "IXPD-9"        "tiny-9"        "chain-9"       "CD-9"         
...
Fitting fL = 0, usekernel = TRUE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "CD"            "numClasses-1"  "IXPD-1"        "numPackages-1" "tiny-1"        "chain-1"       "CD-1"          "numClasses-2"  "IXPD-2"        "tiny-2"       
[11] "chain-2"       "CD-2"          "numClasses-3"  "IXPD-3"        "tiny-3"        "chain-3"       "CD-3"          "numClasses-4"  "IXPD-4"        "numPackages-4"
[21] "tiny-4"        "chain-4"       "CD-4"          "numClasses-5"  "IXPD-5"        "numPackages-5" "tiny-5"        "chain-5"       "CD-5"          "numClasses-6" 
[31] "IXPD-6"        "numPackages-6" "tiny-6"        "chain-6"       "CD-6"          "numClasses-7"  "IXPD-7"        "tiny-7"        "chain-7"       "CD-7"         
[41] "numClasses-8"  "IXPD-8"        "tiny-8"        "chain-8"       "CD-8"          "numClasses-9"  "IXPD-9"        "tiny-9"        "chain-9"       "CD-9"         
...
Fitting fL = 0, usekernel = TRUE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "UD"            "numClasses-1"  "IXPD-1"        "numPackages-1" "tiny-1"        "chain-1"       "CD-1"          "numClasses-2"  "IXPD-2"        "tiny-2"       
[11] "chain-2"       "CD-2"          "numClasses-3"  "IXPD-3"        "tiny-3"        "chain-3"       "CD-3"          "numClasses-4"  "IXPD-4"        "numPackages-4"
[21] "tiny-4"        "chain-4"       "CD-4"          "numClasses-5"  "IXPD-5"        "numPackages-5" "tiny-5"        "chain-5"       "CD-5"          "numClasses-6" 
[31] "IXPD-6"        "numPackages-6" "tiny-6"        "chain-6"       "CD-6"          "numClasses-7"  "IXPD-7"        "tiny-7"        "chain-7"       "CD-7"         
[41] "numClasses-8"  "IXPD-8"        "tiny-8"        "chain-8"       "CD-8"          "numClasses-9"  "IXPD-9"        "tiny-9"        "chain-9"       "CD-9"         
...
Fitting fL = 0, usekernel = TRUE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "IXPD"          "numClasses-1"  "IXPD-1"        "numPackages-1" "tiny-1"        "chain-1"       "CD-1"          "numClasses-2"  "IXPD-2"        "tiny-2"       
[11] "chain-2"       "CD-2"          "numClasses-3"  "IXPD-3"        "tiny-3"        "chain-3"       "CD-3"          "numClasses-4"  "IXPD-4"        "numPackages-4"
[21] "tiny-4"        "chain-4"       "CD-4"          "numClasses-5"  "IXPD-5"        "numPackages-5" "tiny-5"        "chain-5"       "CD-5"          "numClasses-6" 
[31] "IXPD-6"        "numPackages-6" "tiny-6"        "chain-6"       "CD-6"          "numClasses-7"  "IXPD-7"        "tiny-7"        "chain-7"       "CD-7"         
[41] "numClasses-8"  "IXPD-8"        "tiny-8"        "chain-8"       "CD-8"          "numClasses-9"  "IXPD-9"        "tiny-9"        "chain-9"       "CD-9"         
...
Aggregating results
Selecting tuning parameters
Fitting mtry = 64 on full training set
[1] "Model: rf"
 [1] "UD"            "numClasses-1"  "IXPD-1"        "numPackages-1" "tiny-1"        "chain-1"       "CD-1"          "numClasses-2"  "IXPD-2"        "tiny-2"       
[11] "chain-2"       "CD-2"          "numClasses-3"  "IXPD-3"        "tiny-3"        "chain-3"       "CD-3"          "numClasses-4"  "IXPD-4"        "numPackages-4"
[21] "tiny-4"        "chain-4"       "CD-4"          "numClasses-5"  "IXPD-5"        "numPackages-5" "tiny-5"        "chain-5"       "CD-5"          "numClasses-6" 
...
```{r only the best model of C5.0 execution}


    #pkg.dataset.nzv.binary <- as.data.frame(cbind(pkg.dataset.nzv[c("star", "circle", "clique", "chain", "CD", "UD", "HL", "IXPD")],
pkg.dataset.nzv.binary <- as.data.frame(cbind(pkg.dataset.nzv[c("star","tiny", "chain", "CD", "UD", "IXPD")],
                                              ifelse(pkg.dataset.nzv[grep("-", names(pkg.dataset.nzv))]<1,0,1)))
# modelLookup("C5.0")
# models <- c("C5.0") # "C5.0" "logreg" has some issues, should be tested outside
# set.seed(825)
# trainC50 <- run50CV(pkg.dataset.nzv.binary, models, targets, "junit-")

# library(rJava)
modelLookup("JRip")
models <- c("JRip")
set.seed(825)
trainjrip <- run50CV(pkg.dataset.nzv.binary, models, targets, "junit-")
# write.csv(pkg.dataset.nzv.binary, "junit-cm-lagged-week.csv")
...
c50.5 <- trainjrip$`7`$finalModel #ud
c50.6 <- trainjrip$`9`$finalModel #ixpd

write(as.character(summary(c50.1)),"data/rule/junit-jrip-month-c50.star.txt")
write(as.matrix(scan(text=.jcall(c50.1$classifier, "S", "toString") ,sep="\n", what="") )[
    -c(1:2, 6), ,drop=FALSE],"data/rule/junit-jrip-month-c50.star.txt",append=TRUE)
write(as.character(summary(c50.2)),"data/rule/junit-jrip-month-c50.tiny.txt")
write(as.matrix(scan(text=.jcall(c50.2$classifier, "S", "toString") ,sep="\n", what="") )[
    -c(1:2, 6), ,drop=FALSE],"data/rule/junit-jrip-month-c50.tiny.txt",append=TRUE)
write(as.character(summary(c50.3)),"data/rule/junit-jrip-month-c50.chain.txt")
write(as.matrix(scan(text = .jcall(c50.3$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/junit-jrip-month-c50.chain.txt", append = TRUE)
write(as.character(summary(c50.4)), "data/rule/junit-jrip-month-c50.cd.txt")
write(as.matrix(scan(text = .jcall(c50.4$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/junit-jrip-month-c50.cd.txt", append = TRUE)
write(as.character(summary(c50.5)),"data/rule/junit-jrip-month-c50.ud.txt")
write(as.matrix(scan(text=.jcall(c50.5$classifier, "S", "toString") ,sep="\n", what="") )[
    -c(1:2, 6), ,drop=FALSE],"data/rule/junit-jrip-month-c50.ud.txt",append=TRUE)
write(as.character(summary(c50.6)),"data/rule/junit-jrip-month-c50.ixpd.txt")
write(as.matrix(scan(text=.jcall(c50.6$classifier, "S", "toString") ,sep="\n", what="") )[
    -c(1:2, 6), ,drop=FALSE],"data/rule/junit-jrip-month-c50.ixpd.txt",append=TRUE)
#write(as.character(summary(c50.7)),"data/rule/junit-jrip-month-c50.UD.txt")
#write(as.matrix(scan(text=.jcall(c50.76$classifier, "S", "toString") ,sep="\n", what="") )[
#    -c(1:2, 6), ,drop=FALSE],"data/rule/junit-jrip-month-c50.UD.txt",append=TRUE)
#write(as.character(summary(c50.7)),"data/rule/junit-jrip-month-c50.HL.txt")
#write(as.matrix(scan(text=.jcall(c50.7$classifier, "S", "toString") ,sep="\n", what="") )[
#    -c(1:2, 6), ,drop=FALSE],"data/rule/junit-jrip-month-c50.HL.txt",append=TRUE)
#write(as.character(summary(c50.8)),"data/rule/junit-jrip-month-c50.IXPD.txt")
...
# library(plyr)
# library(stringr)
files <- list.files(path = "data/ml-results/junit/month", pattern = ".csv")
# temp <- lapply(files, read.csv,header=TRUE,sep=",")
readFromCsv <- function(filename, header=TRUE, sep=","){
    ret <- read.csv(paste0("data/ml-results/junit/month/",filename),header,sep)
...
run50CV.smote <- function(lagged, .models, .shapes, .outfilePrefix = "") {
  # Generate all combinations of targets and models to test
  experiments <- expand.grid(shape = .shapes, model = .models)
  print(experiments)
  alply(experiments, .margins = 1, function(experiment) {#aaply(array) alply(liste)
...
```{r only the best model of C5.0 execution smote}
...
models <- c("JRip")
set.seed(825)
trainjrip <- run50CV.smote(pkg.dataset.nzv.binary, models, targets, "junit-")
# write.csv(pkg.dataset.nzv.binary, "junit-cm-lagged-week.csv")
...
write(as.character(summary(c50.1)),"data/rule.smote/junit-jrip-month-c50.star.txt")
write(as.matrix(scan(text=.jcall(c50.1$classifier, "S", "toString") ,sep="\n", what="") )[
    -c(1:2, 6), ,drop=FALSE],"data/rule.smote/junit-jrip-month-c50.star.txt",append=TRUE)
write(as.character(summary(c50.2)),"data/rule.smote/junit-jrip-month-c50.tiny.txt")
write(as.matrix(scan(text=.jcall(c50.2$classifier, "S", "toString") ,sep="\n", what="") )[
    -c(1:2, 6), ,drop=FALSE],"data/rule.smote/junit-jrip-month-c50.tiny.txt",append=TRUE)
write(as.character(summary(c50.3)),"data/rule.smote/junit-jrip-month-c50.chain.txt")
write(as.matrix(scan(text = .jcall(c50.3$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/junit-jrip-month-c50.chain.txt", append = TRUE)
write(as.character(summary(c50.4)), "data/rule.smote/junit-jrip-month-c50.cd.txt")
write(as.matrix(scan(text = .jcall(c50.4$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/junit-jrip-month-c50.cd.txt", append = TRUE)
write(as.character(summary(c50.5)),"data/rule.smote/junit-jrip-month-c50.ud.txt")
write(as.matrix(scan(text=.jcall(c50.5$classifier, "S", "toString") ,sep="\n", what="") )[
    -c(1:2, 6), ,drop=FALSE],"data/rule.smote/junit-jrip-month-c50.ud.txt",append=TRUE)
write(as.character(summary(c50.6)),"data/rule.smote/junit-jrip-month-c50.ixpd.txt")
write(as.matrix(scan(text=.jcall(c50.6$classifier, "S", "toString") ,sep="\n", what="") )[
    -c(1:2, 6), ,drop=FALSE],"data/rule.smote/junit-jrip-month-c50.ixpd.txt",append=TRUE)
#write(as.character(summary(c50.7)),"data/rule.smote/junit-jrip-month-c50.UD.txt")
#write(as.matrix(scan(text=.jcall(c50.76$classifier, "S", "toString") ,sep="\n", what="") )[
#    -c(1:2, 6), ,drop=FALSE],"data/rule.smote/junit-jrip-month-c50.UD.txt",append=TRUE)
#write(as.character(summary(c50.7)),"data/rule.smote/junit-jrip-month-c50.HL.txt")
#write(as.matrix(scan(text=.jcall(c50.7$classifier, "S", "toString") ,sep="\n", what="") )[
#    -c(1:2, 6), ,drop=FALSE],"data/rule.smote/junit-jrip-month-c50.HL.txt",append=TRUE)
#write(as.character(summary(c50.8)),"data/rule.smote/junit-jrip-month-c50.IXPD.txt")
#write(as.matrix(scan(text=.jcall(c50.8$classifier, "S", "toString") ,sep="\n", what="") )[
#    -c(1:2, 6), ,drop=FALSE],"data/rule.smote/junit-jrip-month-c50.IXPD.txt",append=TRUE)
```
...
pkg.merged.df.sum <- ddply(pkg.merged.df[setdiff(names(pkg.merged.df), c("version", "commit_time"))],
    ...
    write.csv(file = "junit-as-evolution.csv", pkg.merged.df.sum)
```