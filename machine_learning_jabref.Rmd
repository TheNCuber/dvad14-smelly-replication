...
# Manually add 'HL' column (nzv for our data), as it is a metric
pkg.dataset.nzv <- pkg.dataset[, -setdiff(nearZeroVar(pkg.dataset), grep('^HL$', colnames(pkg.dataset)))]
...
files <- list.files(path = "data/ml-results/jabref/month", pattern = ".csv")
readFromCsv <- function(filename, header = TRUE, sep = ",") {
  ret <- read.csv(paste0("data/ml-results/jabref/month/", filename), header, sep)
