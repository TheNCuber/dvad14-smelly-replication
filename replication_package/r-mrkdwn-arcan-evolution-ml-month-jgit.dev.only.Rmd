...
pkg.lagged <- ddply(pkg.merged.df, .(package), function(pkg) {
  sorted <- pkg[order(pkg[["commit_date"]], decreasing = TRUE), ]
    lag.df(sorted, 12, colnames = setdiff(names(sorted), c("package", "commit_date", "star", "circle", "tiny", "clique", "chain", "CD", "UD", "HL", "IXPD")))
}, .progress = "text")
```
...
set.seed(825)
confusionMatricesNB <- runCV(pkg.dataset.nzv, models, targets, "dev.only.jgit-month-")
write.csv(confusionMatricesNB, "dev.only.jgit-month-cm-ALL-nb.csv")
#```

#```{r train C5.0 and all targets}
models <- c("C5.0") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesC50 <- runCV(pkg.dataset.nzv, models, targets, "dev.only.jgit-month-")
write.csv(confusionMatricesC50, "dev.only.jgit-month-cm-ALL-c50.csv")
#```
...
set.seed(825)
confusionMatricesRF <- runCV(pkg.dataset.nzv, models, targets, "dev.only.jgit-month-")
write.csv(confusionMatricesRF, "dev.only.jgit-month-cm-ALL-rf.csv")
#```

#```{r write all confusion matrices}
write.csv(rbind(confusionMatricesNB, confusionMatricesC50, confusionMatricesRF), "dev.only.jgit-month-cm-ALL-ALL.csv")
```
...
models <- c("nb") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesNB <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.dev.only.jgit-month-")
write.csv(confusionMatricesNB, "smote.dev.only.jgit-month-cm-ALL-nb.csv")
#```

#```{r train C5.0 and all targets}
models <- c("C5.0") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesC50 <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.dev.only.jgit-month-")
write.csv(confusionMatricesC50, "smote.dev.only.jgit-month-cm-ALL-c50.csv")
#```
...
models <- c("rf") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesRF <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.dev.only.jgit-month-")
write.csv(confusionMatricesRF, "smote.dev.only.jgit-month-cm-ALL-rf.csv")
#```

#```{r write all confusion matrices}
write.csv(rbind(confusionMatricesNB, confusionMatricesC50, confusionMatricesRF), "smote.dev.only.jgit-month-cm-ALL-ALL.csv")
```
...
set.seed(825)
    ...
    grid <- expand.grid(sigma = c(.05, .07, .1, .15, .17, .2, .25, .3, .35, .4), C = c(.5, 1.0, 1.25, 1.5, 2, 2.5, 3, 3.5,5,10))
confusionMatricesSVMRadial <- runCV.svm(pkg.dataset.nzv, grid, models, targets, "dev.only.jgit-month-")
write.csv(confusionMatricesSVMRadial, "dev.only.jgit-month-cm-ALL-svm-radial.csv")
#```
...
confusionMatricesSVMlinear <- runCV.svm(pkg.dataset.nzv, grid, models, targets, "dev.only.jgit-month-")
write.csv(confusionMatricesSVMlinear, "dev.only.jgit-month-cm-ALL-svm-linear.csv")
```



[1] "Model: nb"
 [1] "star"                 "max.file.changed-1"   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "max.file.changed-2"  
[10] "numjavafile-2"        "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "max.file.changed-3"   "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "max.file.changed-4"   "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "max.file.changed-5"   "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "max.file.changed-6"   "numjavafile-6"        "nummail-6"           
[37] "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "max.file.changed-7"   "numjavafile-7"        "nummail-7"            "sum.added.total-7"    "sum.removed.total-7" 
[46] "sum.added.file-7"     "sum.removed.file-7"   "max.file.changed-8"   "numjavafile-8"        "nummail-8"            "sum.added.total-8"    "sum.removed.total-8"  "sum.added.file-8"     "sum.removed.file-8"  
[55] "max.file.changed-9"   "numjavafile-9"        "nummail-9"            "sum.added.total-9"    "sum.removed.total-9"  "sum.added.file-9"     "sum.removed.file-9"   "max.file.changed-10"  "numjavafile-10"      
[64] "nummail-10"           "sum.added.total-10"   "sum.removed.total-10" "sum.added.file-10"    "sum.removed.file-10"  "max.file.changed-11"  "numjavafile-11"       "nummail-11"           "sum.added.total-11"  
[73] "sum.removed.total-11" "sum.added.file-11"    "sum.removed.file-11"  "max.file.changed-12"  "numjavafile-12"       "nummail-12"           "sum.added.total-12"   "sum.removed.total-12" "sum.added.file-12"   
[82] "sum.removed.file-12" 
Aggregating results
Selecting tuning parameters
Fitting fL = 0, usekernel = FALSE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "circle"               "max.file.changed-1"   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "max.file.changed-2"  
[10] "numjavafile-2"        "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "max.file.changed-3"   "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "max.file.changed-4"   "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "max.file.changed-5"   "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "max.file.changed-6"   "numjavafile-6"        "nummail-6"           
[37] "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "max.file.changed-7"   "numjavafile-7"        "nummail-7"            "sum.added.total-7"    "sum.removed.total-7" 
...
[1] "Model: nb"
 [1] "clique"               "max.file.changed-1"   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "max.file.changed-2"  
[10] "numjavafile-2"        "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "max.file.changed-3"   "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "max.file.changed-4"   "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "max.file.changed-5"   "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "max.file.changed-6"   "numjavafile-6"        "nummail-6"           
[37] "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "max.file.changed-7"   "numjavafile-7"        "nummail-7"            "sum.added.total-7"    "sum.removed.total-7" 
...
Fitting fL = 0, usekernel = TRUE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "chain"                "max.file.changed-1"   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "max.file.changed-2"  
[10] "numjavafile-2"        "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "max.file.changed-3"   "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "max.file.changed-4"   "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "max.file.changed-5"   "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "max.file.changed-6"   "numjavafile-6"        "nummail-6"           
[37] "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "max.file.changed-7"   "numjavafile-7"        "nummail-7"            "sum.added.total-7"    "sum.removed.total-7" 
...
Fitting fL = 0, usekernel = TRUE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "CD"                   "max.file.changed-1"   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "max.file.changed-2"  
[10] "numjavafile-2"        "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "max.file.changed-3"   "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "max.file.changed-4"   "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "max.file.changed-5"   "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "max.file.changed-6"   "numjavafile-6"        "nummail-6"           
[37] "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "max.file.changed-7"   "numjavafile-7"        "nummail-7"            "sum.added.total-7"    "sum.removed.total-7" 
...
Fitting fL = 0, usekernel = FALSE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "UD"                   "max.file.changed-1"   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "max.file.changed-2"  
[10] "numjavafile-2"        "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "max.file.changed-3"   "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "max.file.changed-4"   "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "max.file.changed-5"   "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "max.file.changed-6"   "numjavafile-6"        "nummail-6"           
[37] "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "max.file.changed-7"   "numjavafile-7"        "nummail-7"            "sum.added.total-7"    "sum.removed.total-7" 
...
Fitting fL = 0, usekernel = FALSE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "HL"                   "max.file.changed-1"   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "max.file.changed-2"  
[10] "numjavafile-2"        "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "max.file.changed-3"   "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "max.file.changed-4"   "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "max.file.changed-5"   "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "max.file.changed-6"   "numjavafile-6"        "nummail-6"           
[37] "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "max.file.changed-7"   "numjavafile-7"        "nummail-7"            "sum.added.total-7"    "sum.removed.total-7" 
...
Fitting fL = 0, usekernel = FALSE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "IXPD"                 "max.file.changed-1"   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "max.file.changed-2"  
[10] "numjavafile-2"        "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "max.file.changed-3"   "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "max.file.changed-4"   "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "max.file.changed-5"   "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "max.file.changed-6"   "numjavafile-6"        "nummail-6"           
[37] "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "max.file.changed-7"   "numjavafile-7"        "nummail-7"            "sum.added.total-7"    "sum.removed.total-7" 
...
Aggregating results
Selecting tuning parameters
Fitting mtry = 81 on full training set
[1] "Model: rf"
 [1] "UD"                   "max.file.changed-1"   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "max.file.changed-2"  
[10] "numjavafile-2"        "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "max.file.changed-3"   "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "max.file.changed-4"   "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
...
tuneGridf <- function(currentShape){
  ...
  if(currentShape=="chain"){
      data.frame(trials = 1, model = "tree", winnow = TRUE)
  }
  if(currentShape=="CD"){
      data.frame(trials = 10, model = "rules", winnow = FALSE)
  }
  if(currentShape=="UD"){
      data.frame(trials = 1, model = "rules", winnow = TRUE)
  }
  if(currentShape=="HL"){
...
models <- c("JRip")
set.seed(825)
trainjrip <- run50CV(pkg.dataset.nzv.binary, models, targets, "dev.only.jgit-")
# write.csv(pkg.dataset.nzv.binary, "tomcat-cm-lagged-week.csv")
    ...
    write(as.character(summary(c50.1)), "data/rule/dev.only.jgit-jrip-month-c50.star.txt")
    write(as.matrix(scan(text = .jcall(c50.1$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.only.jgit-jrip-month-c50.star.txt", append = TRUE)
    write(as.character(summary(c50.2)), "data/rule/dev.only.jgit-jrip-month-c50.circle.txt")
    write(as.matrix(scan(text = .jcall(c50.2$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.only.jgit-jrip-month-c50.circle.txt", append = TRUE)
    write(as.character(summary(c50.3)), "data/rule/dev.only.jgit-jrip-month-c50.clique.txt")
    write(as.matrix(scan(text = .jcall(c50.3$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.only.jgit-jrip-month-c50.clique.txt", append = TRUE)
    write(as.character(summary(c50.4)), "data/rule/dev.only.jgit-jrip-month-c50.chain.txt")
    write(as.matrix(scan(text = .jcall(c50.4$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.only.jgit-jrip-month-c50.chain.txt", append = TRUE)
    write(as.character(summary(c50.5)), "data/rule/dev.only.jgit-jrip-month-c50.cd.txt")
    write(as.matrix(scan(text = .jcall(c50.5$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.only.jgit-jrip-month-c50.cd.txt", append = TRUE)
    write(as.character(summary(c50.6)), "data/rule/dev.only.jgit-jrip-month-c50.ud.txt")
    write(as.matrix(scan(text = .jcall(c50.6$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.only.jgit-jrip-month-c50.ud.txt", append = TRUE)
    write(as.character(summary(c50.7)), "data/rule/dev.only.jgit-jrip-month-c50.hl.txt")
    write(as.matrix(scan(text = .jcall(c50.7$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.only.jgit-jrip-month-c50.hl.txt", append = TRUE)
    write(as.character(summary(c50.8)), "data/rule/dev.only.jgit-jrip-month-c50.ixpd.txt")
    write(as.matrix(scan(text = .jcall(c50.8$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.only.jgit-jrip-month-c50.ixpd.txt", append = TRUE)
...
# write.csv(pkg.dataset.nzv.binary, "tomcat-cm-lagged-week.csv")
    ...
    c50.7 <- trainjrip$`8`$finalModel #hl
    c50.8 <- trainjrip$`9`$finalModel #ixpd
    write(as.character(summary(c50.1)), "data/rule.smote/dev.only.jgit-jrip-month-c50.star.txt")
    write(as.matrix(scan(text = .jcall(c50.1$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.only.jgit-jrip-month-c50.star.txt", append = TRUE)
    write(as.character(summary(c50.2)), "data/rule.smote/dev.only.jgit-jrip-month-c50.circle.txt")
    write(as.matrix(scan(text = .jcall(c50.2$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.only.jgit-jrip-month-c50.circle.txt", append = TRUE)
    write(as.character(summary(c50.3)), "data/rule.smote/dev.only.jgit-jrip-month-c50.clique.txt")
    write(as.matrix(scan(text = .jcall(c50.3$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.only.jgit-jrip-month-c50.clique.txt", append = TRUE)
    write(as.character(summary(c50.4)), "data/rule.smote/dev.only.jgit-jrip-month-c50.chain.txt")
    write(as.matrix(scan(text = .jcall(c50.4$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.only.jgit-jrip-month-c50.chain.txt", append = TRUE)
    write(as.character(summary(c50.5)), "data/rule.smote/dev.only.jgit-jrip-month-c50.cd.txt")
    write(as.matrix(scan(text = .jcall(c50.5$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.only.jgit-jrip-month-c50.cd.txt", append = TRUE)
    write(as.character(summary(c50.6)), "data/rule.smote/dev.only.jgit-jrip-month-c50.ud.txt")
    write(as.matrix(scan(text = .jcall(c50.6$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.only.jgit-jrip-month-c50.ud.txt", append = TRUE)
    write(as.character(summary(c50.7)), "data/rule.smote/dev.only.jgit-jrip-month-c50.hl.txt")
    write(as.matrix(scan(text = .jcall(c50.7$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.only.jgit-jrip-month-c50.hl.txt", append = TRUE)
    write(as.character(summary(c50.8)), "data/rule.smote/dev.only.jgit-jrip-month-c50.ixpd.txt")
    write(as.matrix(scan(text = .jcall(c50.8$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.only.jgit-jrip-month-c50.ixpd.txt", append = TRUE)
