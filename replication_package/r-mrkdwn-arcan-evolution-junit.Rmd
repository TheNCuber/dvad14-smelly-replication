---
title: "AS evolution with arcan junit"
author: "RR"
...
# classmetrics <- read.csv(file="data/junit/CM.csv",colClasses=c('character','myDate','character','numeric','numeric','numeric','numeric'),head=TRUE,sep=",")
...
#to.timestamp.fromint <- function(d) as.POSIXct(d, origin = "1970-01-01", tz = "GMT")
# library(data.table)
classmetrics<-fread("data/junit/CM.csv")
...
# pkgmetrics <- read.csv(file="data/junit/PM.csv",colClasses=c('character','myDate','character','numeric','numeric','numeric','numeric','numeric'),head=TRUE,sep=",")
pkgmetrics <- fread("data/junit/PM.csv")
...
# hublike <- read.csv(file="data/junit/HL.csv",colClasses=c('character','myDate','character','numeric','numeric','numeric'),head=TRUE,sep=",")
hublike <- fread("data/junit/HL.csv")
...
# unstabledep <- read.csv(file="data/junit/UD.csv", colClasses=c('character', 'myDate', 'character', 'numeric', 'character', 'numeric'), head=TRUE, sep=",")
library(data.table)
unstabledep <- fread(file="data/junit/UD.csv")
...
clcycle <- fread("data/junit/classCyclesShapeTable.csv")
...
pkgcycle <- fread("data/junit/packageCyclesShapeTable.csv")
...
```{r Reading IXPD csv}
ixpd <- fread(file="data/junit/IXPD.csv")
#library(stringr)
...
ixpd <- ixpd[, java.out := gsub("^.*\\.(main|java)\\.", "", java.out),]
ixpd <- ixpd[, java.in := gsub("^.*\\.(main|java)\\.", "", java.in),]
...
```{r developers info}
class.dev = fread("data/junit/development.csv")
class.dev$commit_time <- to.timestamp.fromint(class.dev$commit_time)
class.dev <- class.dev[, javafile := gsub("^.*\\.(main|java)\\.", "", javafile),]
...
fwrite(pkg.metric.CD.HL.UD.ixpd, "cache/junit/package_merged.csv", append = FALSE)
fwrite(class.cycle.metric.ixpd.HL.UD.merging, "cache/junit/class_merged.csv", append = FALSE)
```
...
```{r write merged datasets to file}
fwrite(pkg.metric.CD.HL.UD.ixpd.dev, "cache/junit/package_merged.dev.csv", append = FALSE)
```
