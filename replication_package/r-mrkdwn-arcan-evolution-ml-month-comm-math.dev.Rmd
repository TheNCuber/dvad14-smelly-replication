    ...
    runCV.smote <- function(lagged, .models, .shapes, .outfilePrefix = "") {
        ...
        adply(experiments, .margins = 1, function(experiment) {
            ...
            if (currentShape %in% names(lagged)) {
                ...
                print(paste("Model:", currentModel))
                print(names(lagged.train))
                #lagged.train <- SMOTE(as.formula(paste(currentShape, "~ .")), data = lagged.train, perc.over = 500, perc.under = 100)
                fit <- train(as.formula(paste(currentShape, "~ .")),
                   data = lagged.train,
                   method = currentModel,
...
set.seed(825)
confusionMatricesNB <- runCV(pkg.dataset.nzv, models, targets, "dev.commons-math-month-")
write.csv(confusionMatricesNB, "dev.commons-math-month-cm-ALL-nb.csv")
#```

#```{r train C5.0 and all targets}
models <- c("C5.0") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesC50 <- runCV(pkg.dataset.nzv, models, targets, "dev.commons-math-month-")
write.csv(confusionMatricesC50, "dev.commons-math-month-cm-ALL-c50.csv")
#```
...
set.seed(825)
confusionMatricesRF <- runCV(pkg.dataset.nzv, models, targets, "dev.commons-math-month-")
write.csv(confusionMatricesRF, "dev.commons-math-month-cm-ALL-rf.csv")
#```

#```{r write all confusion matrices}
write.csv(rbind(confusionMatricesNB, confusionMatricesC50, confusionMatricesRF), "dev.commons-math-month-cm-ALL-ALL.csv")
```

```{r train nb and all targets}
models <- c("nb") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesNB <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.dev.commons-math-month-")
write.csv(confusionMatricesNB, "smote.dev.commons-math-month-cm-ALL-nb.csv")
#```
...
models <- c("C5.0") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesC50 <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.dev.commons-math-month-")
write.csv(confusionMatricesC50, "smote.dev.commons-math-month-cm-ALL-c50.csv")
#```
...
models <- c("rf") # "C5.0" "logreg" has some issues, should be tested outside
set.seed(825)
confusionMatricesRF <- runCV.smote(pkg.dataset.nzv, models, targets, "smote.dev.commons-math-month-")
write.csv(confusionMatricesRF, "smote.dev.commons-math-month-cm-ALL-rf.csv")
#```

#```{r write all confusion matrices}
write.csv(rbind(confusionMatricesNB, confusionMatricesC50, confusionMatricesRF), "smote.dev.commons-math-month-cm-ALL-ALL.csv")
```
...
confusionMatricesSVMRadial <- runCV.svm(pkg.dataset.nzv, grid, models, targets, "dev.commons-math-month-")
write.csv(confusionMatricesSVMRadial, "dev.commons-math-month-cm-ALL-svm-radial.csv")
```
...
confusionMatricesSVMlinear <- runCV.svm(pkg.dataset.nzv, grid, models, targets, "dev.commons-math-month-")
write.csv(confusionMatricesSVMlinear, "dev.commons-math-month-cm-ALL-svm-linear.csv")
...
grid <- expand.grid(C = c(0.01,0.05,0.1,0.5,0.75,0.9, 1, 1.1, 1.25))
confusionMatricesSVMlinear <- runCV.svm(pkg.dataset.nzv, grid, models, targets, "dev.commons-math-month-")
write.csv(confusionMatricesSVMlinear, "dev.commons-math-month-cm-ALL-svm-poly.csv")

```

[1] "Model: nb"
 [1] "star"                 "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "CD-1"                 "numjavafile-2"       
[10] "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "sum.removed.file-2"   "CD-2"                 "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "CD-3"                 "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "sum.removed.file-4"   "CD-4"                 "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "sum.removed.file-5"   "CD-5"                
[37] "numjavafile-6"        "nummail-6"            "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "CD-6"                 "numjavafile-7"        "nummail-7"           
[46] "sum.added.total-7"    "sum.removed.total-7"  "sum.added.file-7"     "sum.removed.file-7"   "CD-7"                 "max.file.changed-8"   "numjavafile-8"        "nummail-8"            "sum.added.total-8"   
[55] "sum.removed.total-8"  "sum.added.file-8"     "sum.removed.file-8"   "CD-8"                 "max.file.changed-9"   "numjavafile-9"        "nummail-9"            "sum.added.total-9"    "sum.removed.total-9" 
[64] "sum.added.file-9"     "sum.removed.file-9"   "CD-9"                 "max.file.changed-10"  "numjavafile-10"       "nummail-10"           "sum.added.total-10"   "sum.removed.total-10" "sum.added.file-10"   
[73] "sum.removed.file-10"  "CD-10"                "max.file.changed-11"  "numjavafile-11"       "nummail-11"           "sum.added.total-11"   "sum.removed.total-11" "sum.added.file-11"    "sum.removed.file-11" 
[82] "CD-11"                "max.file.changed-12"  "numjavafile-12"       "nummail-12"           "sum.added.total-12"   "sum.removed.total-12" "sum.added.file-12"    "sum.removed.file-12"  "CD-12"               
Aggregating results
Selecting tuning parameters
Fitting fL = 0, usekernel = TRUE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "tiny"                 "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "CD-1"                 "numjavafile-2"       
[10] "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "sum.removed.file-2"   "CD-2"                 "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "CD-3"                 "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "sum.removed.file-4"   "CD-4"                 "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "sum.removed.file-5"   "CD-5"                
[37] "numjavafile-6"        "nummail-6"            "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "CD-6"                 "numjavafile-7"        "nummail-7"           
...
Fitting fL = 0, usekernel = TRUE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "clique"               "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "CD-1"                 "numjavafile-2"       
[10] "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "sum.removed.file-2"   "CD-2"                 "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "CD-3"                 "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "sum.removed.file-4"   "CD-4"                 "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "sum.removed.file-5"   "CD-5"                
[37] "numjavafile-6"        "nummail-6"            "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "CD-6"                 "numjavafile-7"        "nummail-7"           
...
Fitting fL = 0, usekernel = TRUE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "chain"                "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "CD-1"                 "numjavafile-2"       
[10] "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "sum.removed.file-2"   "CD-2"                 "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "CD-3"                 "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "sum.removed.file-4"   "CD-4"                 "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "sum.removed.file-5"   "CD-5"                
[37] "numjavafile-6"        "nummail-6"            "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "CD-6"                 "numjavafile-7"        "nummail-7"           
...
Fitting fL = 0, usekernel = TRUE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "CD"                   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "CD-1"                 "numjavafile-2"       
[10] "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "sum.removed.file-2"   "CD-2"                 "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "CD-3"                 "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "sum.removed.file-4"   "CD-4"                 "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "sum.removed.file-5"   "CD-5"                
[37] "numjavafile-6"        "nummail-6"            "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "CD-6"                 "numjavafile-7"        "nummail-7"           
...
[1] "Model: nb"
 [1] "UD"                   "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "CD-1"                 "numjavafile-2"       
[10] "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "sum.removed.file-2"   "CD-2"                 "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "CD-3"                 "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "sum.removed.file-4"   "CD-4"                 "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "sum.removed.file-5"   "CD-5"                
[37] "numjavafile-6"        "nummail-6"            "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "CD-6"                 "numjavafile-7"        "nummail-7"           
...
Fitting fL = 0, usekernel = TRUE, adjust = 1 on full training set
[1] "Model: nb"
 [1] "IXPD"                 "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "CD-1"                 "numjavafile-2"       
[10] "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "sum.removed.file-2"   "CD-2"                 "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "CD-3"                 "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
[28] "sum.removed.file-4"   "CD-4"                 "numjavafile-5"        "nummail-5"            "sum.added.total-5"    "sum.removed.total-5"  "sum.added.file-5"     "sum.removed.file-5"   "CD-5"                
[37] "numjavafile-6"        "nummail-6"            "sum.added.total-6"    "sum.removed.total-6"  "sum.added.file-6"     "sum.removed.file-6"   "CD-6"                 "numjavafile-7"        "nummail-7"           
...
Aggregating results
Selecting tuning parameters
Fitting mtry = 89 on full training set
[1] "Model: rf"
 [1] "clique"               "numjavafile-1"        "nummail-1"            "sum.added.total-1"    "sum.removed.total-1"  "sum.added.file-1"     "sum.removed.file-1"   "CD-1"                 "numjavafile-2"       
[10] "nummail-2"            "sum.added.total-2"    "sum.removed.total-2"  "sum.added.file-2"     "sum.removed.file-2"   "CD-2"                 "numjavafile-3"        "nummail-3"            "sum.added.total-3"   
[19] "sum.removed.total-3"  "sum.added.file-3"     "sum.removed.file-3"   "CD-3"                 "numjavafile-4"        "nummail-4"            "sum.added.total-4"    "sum.removed.total-4"  "sum.added.file-4"    
...
models <- c("JRip")
set.seed(825)
trainjrip <- run50CV(pkg.dataset.nzv.binary, models, targets, "dev.commons-math-")
# write.csv(pkg.dataset.nzv.binary, "commons-math-cm-lagged-week.csv")
    ...
    write(as.character(summary(c50.1)), "data/rule/dev.commons-math-jrip-month-c50.star.txt")
    write(as.matrix(scan(text = .jcall(c50.1$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.commons-math-jrip-month-c50.star.txt", append = TRUE)
    write(as.character(summary(c50.2)), "data/rule/dev.commons-math-jrip-month-c50.tiny.txt")
    write(as.matrix(scan(text = .jcall(c50.2$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.commons-math-jrip-month-c50.tiny.txt", append = TRUE)
    write(as.character(summary(c50.3)), "data/rule/dev.commons-math-jrip-month-c50.clique.txt")
    write(as.matrix(scan(text = .jcall(c50.3$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.commons-math-jrip-month-c50.clique.txt", append = TRUE)
    write(as.character(summary(c50.4)), "data/rule/dev.commons-math-jrip-month-c50.chain.txt")
    write(as.matrix(scan(text = .jcall(c50.4$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.commons-math-jrip-month-c50.chain.txt", append = TRUE)
    write(as.character(summary(c50.5)), "data/rule/dev.commons-math-jrip-month-c50.cd.txt")
    write(as.matrix(scan(text = .jcall(c50.5$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.commons-math-jrip-month-c50.cd.txt", append = TRUE)
    write(as.character(summary(c50.6)), "data/rule/dev.commons-math-jrip-month-c50.ud.txt")
    write(as.matrix(scan(text = .jcall(c50.6$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.commons-math-jrip-month-c50.ud.txt", append = TRUE)
    write(as.character(summary(c50.7)), "data/rule/dev.commons-math-jrip-month-c50.ixpd.txt")
    write(as.matrix(scan(text = .jcall(c50.7$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule/dev.commons-math-jrip-month-c50.ixpd.txt", append = TRUE)
...
tuneGridf <- function(currentShape){
    ...
    if (currentShape == "CD") {
        data.frame(trials = 1, model = "rule", winnow = TRUE)
    }
    if (currentShape == "UD") {
        data.frame(trials = 1, model = "rule", winnow = FALSE)
    }
    if (currentShape == "HL") {
        data.frame(trials = 10, model = "rules", winnow = FALSE)
    }
...
# write.csv(pkg.dataset.nzv.binary, "commons-math-cm-lagged-week.csv")
    ...
    write(as.character(summary(c50.1)), "data/rule.smote/dev.commons-math-jrip-month-c50.star.txt")
    write(as.matrix(scan(text = .jcall(c50.1$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.commons-math-jrip-month-c50.star.txt", append = TRUE)
    write(as.character(summary(c50.2)), "data/rule.smote/dev.commons-math-jrip-month-c50.tiny.txt")
    write(as.matrix(scan(text = .jcall(c50.2$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.commons-math-jrip-month-c50.tiny.txt", append = TRUE)
    write(as.character(summary(c50.3)), "data/rule.smote/dev.commons-math-jrip-month-c50.clique.txt")
    write(as.matrix(scan(text = .jcall(c50.3$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.commons-math-jrip-month-c50.clique.txt", append = TRUE)
    write(as.character(summary(c50.4)), "data/rule.smote/dev.commons-math-jrip-month-c50.chain.txt")
    write(as.matrix(scan(text = .jcall(c50.4$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.commons-math-jrip-month-c50.chain.txt", append = TRUE)
    write(as.character(summary(c50.5)), "data/rule.smote/dev.commons-math-jrip-month-c50.cd.txt")
    write(as.matrix(scan(text = .jcall(c50.5$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.commons-math-jrip-month-c50.cd.txt", append = TRUE)
    write(as.character(summary(c50.6)), "data/rule.smote/dev.commons-math-jrip-month-c50.ud.txt")
    write(as.matrix(scan(text = .jcall(c50.6$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.commons-math-jrip-month-c50.ud.txt", append = TRUE)
    write(as.character(summary(c50.7)), "data/rule.smote/dev.commons-math-jrip-month-c50.ixpd.txt")
    write(as.matrix(scan(text = .jcall(c50.7$classifier, "S", "toString"), sep = "\n", what = ""))[
    -c(1:2, 6),, drop = FALSE], "data/rule.smote/dev.commons-math-jrip-month-c50.ixpd.txt", append = TRUE)
